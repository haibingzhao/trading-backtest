...
def run_validation(
    ...
    if "distribution" in v_cfg:
        from backtest.distribution_analysis import (
            identify_drawdown_events,
            monthly_return_distribution,
        )
        dist_cfg = v_cfg["distribution"] if isinstance(v_cfg["distribution"], dict) else {}
        results["distribution"] = {
            "monthly_returns": monthly_return_distribution(equity_curve),
            "drawdown_events": identify_drawdown_events(
                equity_curve,
                min_depth=dist_cfg.get("min_depth", 0.05),
            ),
        }
