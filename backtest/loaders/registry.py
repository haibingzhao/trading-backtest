...
def _ensure_registered() -> None:
    ...
    # Ensure .env is loaded before any loader checks os.getenv() for API keys.
    # python-dotenv is idempotent — safe to call even if runner.py already did it.
    try:
...
# files (``~/.trading-backtest/data-bridge/config.yaml``); its ``markets`` set spans
...
FALLBACK_CHAINS: dict[str, list[str]] = {
    ...
    "hk_equity": ["futu", "eastmoney", "yahoo", "yfinance", "akshare", "local"],
...
def get_loader_cls_with_fallback(source: str) -> Type[Any]:
    ...
    if source in _NO_NETWORK_FALLBACK_SOURCES:
        raise NoAvailableSourceError(
            ...
            f"(~/.trading-backtest/data-bridge/config.yaml) — it must exist and list "
