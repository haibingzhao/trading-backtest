...
LOADER_CACHE_ENV = "TRADING_BACKTEST_DATA_CACHE"
_LOADER_CACHE_ENV_LEGACY = "VIBE_TRADING_DATA_CACHE"
...
def loader_cache_enabled() -> bool:
    ...
    val = os.getenv(LOADER_CACHE_ENV, "").strip().lower()
    if not val:
        val = os.getenv(_LOADER_CACHE_ENV_LEGACY, "").strip().lower()
    return val in _LOADER_CACHE_TRUE_VALUES
...
def loader_cache_path(
    *,
    ...
    return Path.home() / ".trading-backtest" / "cache" / "loaders" / source_dir / f"{key}.parquet"
