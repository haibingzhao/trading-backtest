...
class BaseEngine(ABC):
    ...
    def __init__(self, config: dict):
        ...
        # Engine-level stop rules (optional safety net)
        self._stop_rules = None
        stop_rules_cfg = config.get("stop_rules")
        if stop_rules_cfg:
            from backtest.stop_rules import parse_stop_rules_config
            self._stop_rules = parse_stop_rules_config(stop_rules_cfg)
    ...
    def run_backtest(
        ...
        # 6b. Enhanced benchmark comparison (optional)
        if config.get("benchmark_comparison"):
            from backtest.benchmark_comparison import detailed_benchmark_comparison
            m["comparison"] = detailed_benchmark_comparison(
                equity_series, bench_ret, self.initial_capital, bars_per_year,
            )

        # 6c. Periodic breakdown (default on)
        if config.get("periodic_report", True):
            try:
                from backtest.periodic_metrics import annual_metrics, monthly_summary
                m["by_year"] = annual_metrics(equity_series, self.trades, self.initial_capital, bars_per_year)
                m["by_month"] = monthly_summary(equity_series, self.trades)
            except Exception as exc:
                logger.warning("Periodic metrics failed: %s", exc)

        # 6d. Rolling monitor (opt-in)
        if config.get("rolling_monitor"):
            try:
                from backtest.rolling_monitor import generate_rolling_json
                rolling_data = generate_rolling_json(equity_series, bench_ret, bars_per_year)
                m["rolling"] = rolling_data
                # Write rolling_monitor.json artifact
                rolling_path = run_dir / "artifacts" / "rolling_monitor.json"
                rolling_path.parent.mkdir(parents=True, exist_ok=True)
                rolling_path.write_text(json.dumps(rolling_data, indent=2, ensure_ascii=False), encoding="utf-8")
            except Exception as exc:
                logger.warning("Rolling monitor failed: %s", exc)
        ...
        # 8b. Lot-size impact analysis (after trades.csv is written)
        _is_lot_market = any(
            any(suf in c for c in codes) for suf in (".SH", ".SZ", ".HK")
        )
        if config.get("lot_size_analysis") or _is_lot_market:
            try:
                from backtest.lot_size_analysis import quantify_lot_impact
                trades_csv = run_dir / "artifacts" / "trades.csv"
                if trades_csv.exists():
                    lot_impact = quantify_lot_impact(trades_csv, self.initial_capital)
                    m["lot_size_impact"] = lot_impact
                    # Write lot_size_impact.json
                    lot_path = run_dir / "artifacts" / "lot_size_impact.json"
                    lot_path.write_text(json.dumps(lot_impact, indent=2, ensure_ascii=False), encoding="utf-8")
            except Exception as exc:
                logger.warning("Lot-size analysis failed: %s", exc)
    ...
    def _execute_bars(
        ...
        for i, ts in enumerate(dates):
            ...
            # a2. Engine-level stop rules check (before rebalance)
            if self._stop_rules and self.positions:
                force_close = self._stop_rules.check_positions(
                    self.positions, close_df, ts
                )
                for sym, reason in force_close:
                    if sym in self.positions and ts in close_df.index:
                        bar = data_map.get(sym)
                        if bar is not None and ts in bar.index:
                            open_price = float(bar.loc[ts].get("open", bar.loc[ts].get("close", 0)))
                            price = self.apply_slippage(open_price, -self.positions[sym].direction)
                            self._close_position(sym, price, ts, reason)
    ...
    def _write_artifacts(
        ...
        # Clear stale artifacts from previous runs to avoid leftover files
        # (e.g. ohlcv for symbols no longer in the run, old validation.json).
        if out.exists():
            shutil.rmtree(out)
        ...
        # HTML visual report (chart_data.json + report.html)
        try:
